...
import { ArrowLeft, Instagram, Twitter, Music, Apple, Diamond } from "lucide-react"

export default function Contact({ onBack }) {
  
  useEffect(() => {
    // Remove any existing script to prevent duplicates
    const existingScript = document.querySelector('script[src="https://letterbird.co/embed/v1.js"]');
    if (existingScript) {
      console.log("FOund script to Prevent");
    ...
    script.setAttribute('data-letterbirduser', 'lowpolybrainblasts');
    script.setAttribute('data-targetelement', '#letterbird-form-container');
    script.setAttribute('data-showheader', 'true');
    script.async = true;
    script.id = 'letterbird-script-boo';
    
    document.body.appendChild(script);

    console.log(document.querySelector('script[src="https://letterbird.co/embed/v1.js"]'));

    // Clean up function to remove script when component unmounts
    return () => {
      const scriptToRemove = document.querySelector('script[src="https://letterbird.co/embed/v1.js"]');
      if (scriptToRemove) {
        console.log("FOund script to remove");
  ...
  return (
    ...
    >
      ...
      <div id="contact-icons" className="flex items-center justify-center gap-8 my-8">
        <div className="w-6 h-6 md:w-6 md:h-6 fill-[#E6E5DB]">
      ...
      <div id="contact-form" className="flex flex-col items-center justify-center gap-4 p-4 md:p-8 w-full">
        {/* This div will be the target for the Letterbird form */}
        <div id="letterbird-form-container"  className="w-full"></div>
      </div>
    </motion.div>
  )
}
